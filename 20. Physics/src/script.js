...
const createObject = (size, position, type) => {
    const geometry = (type === 'sphere') ? sphereGeometry : cubeGeometry;
    const mesh = new THREE.Mesh(geometry, material)
    mesh.scale.set(size, size, size);
    mesh.castShadow = true;
    mesh.position.copy(position)
    scene.add(mesh)
    const shape = (type === 'sphere') ? new CANNON.Sphere(size) : new CANNON.Box(new CANNON.Vec3(size * 0.5, size * 0.5, size * 0.5))
...
debugObject.createSphere = () => {
    createObject(Math.random(), { x: 0, y: 3, z: 0 }, 'sphere')
}
debugObject.createBox = () => {
    createObject(Math.random(), { x: 0, y: 3, z: 0 })
}
